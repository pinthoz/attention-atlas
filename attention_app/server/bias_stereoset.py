...
def register_stereoset_handlers(
    ...
    def stereoset_overview():
        ...
        _benchmark_help = (
            ...
            f"<div style='{_TR}'><span style='{_TD};color:#60a5fa;'>●</span>"
            f"<span>Each item is a sentence with three completions: <b>stereotyped</b>, <b>anti-stereotyped</b>, and <b>unrelated</b> (meaningless). The model should prefer the meaningful completions over the unrelated one; an unbiased model shows <b>no preference</b> between the stereotyped and anti-stereotyped completions.</span></div>"
            f"<hr style='{_TS}'/>"
            ...
            f"<span><span style='{_TBA}'>ICAT</span> Idealized CAT (Context Association Test) score - composite score that rewards both low bias (SS near 50%) and high LM quality (high LMS). <b>Max = 100.</b></span></div>"
            f"<hr style='{_TS}'/>"
    ...
    def stereoset_demographic_slices():
        ...
        def _render_single(mk, container_suffix=""):
            ...
            summary_html = (
                ...
                '<div style="font-size:10px;color:#94a3b8;font-style:italic;margin-top:8px;text-align:center;">'
                '* SS colour bands (&lt; 40 anti-stereo, 40–60 near-parity, &gt; 60 stereo) follow the Nadeem et al. (2021) literature convention, with SS=50 as parity. Not empirically calibrated to v9.'
                '</div></div>'
            )
    ...
    def stereoset_head_sensitivity():
        ...
        header_args = (
            ...
            f"sensitivity(l,h) = mean_f Var[ mean_f(l,h | cat) ]</div>"
            f"<div style='{_TN}; margin-bottom:4px;'>variance of per-category means, averaged over all of the head's features (GAM, AttMap, Spec); categories: gender · race · religion · profession</div>"
            f"<hr style='{_TS}'>"
