            ...
            if label["is_biased"]:
                ...
                if current_span is not None:
                    ...
                    spans.append(BiasSpan(
                        start_idx=current_span["start"],
                        end_idx=current_span["end"],
                        tokens=current_span["tokens"],
                        bias_types=list(current_span["bias_types"]),
                        confidence=1.0,  # Rule-based, so confidence is high
                        explanation=" | ".join(filter(None, current_span["explanations"]))
                    ))
                    current_span = None

        # Don't forget last span if still open
        if current_span is not None:
            spans.append(BiasSpan(
                start_idx=current_span["start"],
                end_idx=current_span["end"],
