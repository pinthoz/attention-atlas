...
Usage (run from the repository root):
    python attention_app/bias/analysis/attention_importance_reverse.py \
        --backbone bert \
...
# This script lives in attention_app/bias/analysis/, so it needs on sys.path:
#   - its own dir          -> sibling scripts (span_faithfulness_eval)
#   - ../training          -> gus_net_training, gus_net_training_paper
#   - ../  (bias/)         -> runtime modules (head_ablation, attention_bias,
#                             integrated_gradients) and the stereoset package
#   - the repository root
_SCRIPT_DIR = Path(__file__).resolve().parent
_BIAS_DIR = _SCRIPT_DIR.parent
sys.path.insert(0, str(_SCRIPT_DIR))
sys.path.insert(0, str(_BIAS_DIR / "training"))
sys.path.insert(0, str(_BIAS_DIR))
sys.path.insert(0, str(_BIAS_DIR.parent.parent))
